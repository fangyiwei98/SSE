...
class Reins(nn.Module):
    ...
    def create_model(self):
        ...
        self.scale = nn.Parameter(torch.tensor(self.scale_init))    # 残差缩放因子
        self.mlp_token2feat = nn.Linear(self.embed_dims, self.embed_dims)   # token特征转换器
        self.mlp_delta_f = nn.Linear(self.embed_dims, self.embed_dims)  # 残差生成器
        val = math.sqrt(
            6.0
            / float(
    ...
    def return_auto(self, feats):   #查询向量生成
    ...
    def forward(
        ...
        feats = feats + delta_feat  # 带缩放的残差连接
        if has_cls_token:
    ...
    def forward_delta_feat(self, feats: Tensor, tokens: Tensor, layers: int) -> Tensor:
        attn = torch.einsum("nbc,mc->nbm", feats, tokens)   # 特征与token点积
        if self.use_softmax:    # 缩放softmax,注意力归一化
        ...
        delta_f = self.mlp_delta_f(delta_f + feats) # 残差网络
        return delta_f
